...
def test_disputed_item_cannot_pass_acceptance(tmp_path):
    s=KnowledgeStore(tmp_path/'db'); p=Principal('a','w','p'); sid=str(uuid.uuid4())
    refs=[{'type':'execution','id':sid}]
    ...
    flow=KnowledgeWorkflow(s,checks,source_resolver=lambda p,refs:{sid:1})
